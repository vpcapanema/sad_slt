  ...
  async function init() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const session = await SLTAdminApi.fetchSession();
        if (!session?.authenticated) {
          throw new Error(
            "O login foi aceito, mas a sessão restrita não pôde ser confirmada.",
          );
        }
    ...
    try {
      const session = await SLTAdminApi.fetchSession();
      if (!submitting && session?.authenticated) {
        location.replace(next);
      }
    } catch {
      /* API indisponível — o formulário exibirá o erro se houver tentativa de login. */
    }
  }
