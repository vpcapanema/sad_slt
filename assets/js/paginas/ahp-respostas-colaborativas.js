(function (global) {
  ...
  function renderExplorer() {
    ...
    var ambienteRows=state.ambientes.filter(function(x){return [x.config_codigo,x.config_tipo,x.id,x.token].join(" ").toLowerCase().indexOf(q)>=0;}).map(function(x,index){var cons=x.consolidacao,selected=a&&x.id===a.id;return '<tr class="collab-round-row '+(selected?'is-selected':'')+'" data-round-id="'+esc(x.id)+'" aria-selected="'+selected+'"><td><input type="radio" name="rodada-ahp" '+(selected?'checked':'')+' aria-label="Selecionar rodada criada em '+esc(date(x.criadoEm))+'"></td><td><span class="collab-status">'+esc(x.status)+'</span></td><td>'+esc(x.id)+'</td><td>'+esc(x.config_tipo)+'</td><td>'+esc(x.config_codigo)+'</td><td>'+esc(x.config_avulsa_id)+'</td><td>'+esc(x.config_portfolio_id)+'</td><td>'+esc(x.token)+'</td><td>'+jsonCell(x.criterios,x.n_criterios+" critério(s)")+'</td><td>'+x.n_criterios+'</td><td>'+jsonCell(x.convites,(x.convites||[]).length+" convite(s)")+'</td><td>'+date(x.valido_ate)+'</td><td>'+date(x.criadoEm)+'</td><td>'+date(x.atualizadoEm)+'</td><td>'+jsonCell(cons&&cons.matriz_consolidada,"Ver matriz")+'</td><td>'+jsonCell(cons&&cons.pesos_consolidados,"Ver pesos")+'</td><td>'+num(cons&&cons.lambda_max,4)+'</td><td>'+num(cons&&cons.indice_consistencia,4)+'</td><td>'+num(cons&&cons.indice_aleatorio,4)+'</td><td>'+num(cons&&cons.razao_consistencia,4)+'</td><td>'+(cons?(cons.consistente?"Sim":"Não"):"—")+'</td><td>'+(cons?cons.respostas_consolidadas:"—")+'</td><td>'+date(cons&&cons.consolidadoEm)+'</td><td><button class="btn btn-secondary btn-sm" data-round-detail="'+esc(x.id)+'">Ver detalhes</button></td></tr>';});
    el("ambiente-table").innerHTML=table([alias("Selecionar","seleção"),alias("Situação da rodada","status"),alias("Identificador da rodada","id"),alias("Tipo de configuração","config_tipo"),alias("Código da configuração","config_codigo"),alias("Configuração avulsa vinculada","config_avulsa_id"),alias("Configuração de portfólio vinculada","config_portfolio_id"),alias("Token de acesso","token"),alias("Critérios da rodada","criterios"),alias("Quantidade de critérios","n_criterios"),alias("Convidados","convites"),alias("Prazo para respostas","valido_ate"),alias("Criada em","criado_em"),alias("Atualizada em","atualizado_em"),alias("Matriz consolidada","matriz_consolidada"),alias("Pesos consolidados","pesos_consolidados"),alias("Lambda máximo (λmax)","lambda_max"),alias("Índice de consistência (IC)","indice_consistencia"),alias("Índice aleatório (IA)","indice_aleatorio"),alias("Razão de consistência (RC)","razao_consistencia"),alias("Resultado consistente","consistente"),alias("Respostas consolidadas","respostas_consolidadas"),alias("Consolidada em","consolidado_em"),alias("Ações","ações")],ambienteRows);
    el("respostas-table").innerHTML=table([alias("Resposta consistente","consistente"),alias("Identificador da resposta","id"),alias("Rodada AHP","ambiente_id"),alias("Participante","nome_completo"),alias("E-mail","email"),alias("Instituição","instituicao"),alias("Matriz de comparação","matriz_comparacao"),alias("Lambda máximo (λmax)","lambda_max"),alias("Índice de consistência (IC)","indice_consistencia"),alias("Índice aleatório (IA)","indice_aleatorio"),alias("Razão de consistência (RC)","razao_consistencia"),alias("Estatísticas adicionais","estatisticas"),alias("Enviada em","enviado_em"),alias("Ações","ações")],filtered.map(function(o){var x=o.x;return '<tr><td><span class="collab-status '+(x.consistente?'':'is-muted')+'">'+(x.consistente?'Sim':'Não')+'</span></td><td>'+esc(x.id)+'</td><td>'+esc(x.ambiente_id)+'</td><td>'+esc(x.nome_completo)+'</td><td>'+esc(x.email)+'</td><td>'+esc(x.instituicao)+'</td><td>'+jsonCell(x.matriz_comparacao,"Ver matriz")+'</td><td>'+num(x.lambda_max,4)+'</td><td>'+num(x.indice_consistencia,4)+'</td><td>'+num(x.indice_aleatorio,4)+'</td><td>'+num(x.razao_consistencia,4)+'</td><td>'+jsonCell(x.estatisticas,"Ver estatísticas")+'</td><td>'+date(x.enviadoEm)+'</td><td><button class="btn btn-secondary btn-sm" data-detail="resposta" data-index="'+o.i+'">Ver detalhes</button></td></tr>'; }));
  ...
  function renderPairs() {
    ...
    host.innerHTML=table(["Concordância","Comparação","Julgamento médio","Dispersão"],p.map(function(x){var level=x.sd<.2?"Alta":x.sd<.5?"Moderada":"Baixa";return '<tr><td><span class="collab-status '+(level==="Alta"?'':'is-muted')+'">'+level+'</span></td><td><strong>'+esc(state.criterios[x.i])+'</strong><br><span>versus '+esc(state.criterios[x.j])+'</span></td><td>'+num(x.gm,3)+'</td><td>'+num(x.sd,3)+'</td></tr>'; }));
  }
