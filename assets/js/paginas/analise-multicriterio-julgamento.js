(function () {
  "use strict";
  var root, id, julgamento, respostas = [], hierarquia, julgamentos = [];
  var prefixMatch = location.pathname.match(/^(.*?)\/restrict\//); var appPrefix = prefixMatch ? prefixMatch[1] : "";
  ...
  function names() { return (julgamento.criterios || []).map(function (c) { return typeof c === "string" ? c : c.criterio || c.nome || c.name || c.codigo; }); }
  function localInput(iso) { var d = new Date(iso); d.setMinutes(d.getMinutes() - d.getTimezoneOffset()); return d.toISOString().slice(0, 10); }
  function deadlineLabel(iso) { return new Date(iso).toLocaleDateString("pt-BR", { timeZone: "America/Sao_Paulo" }); }
  function deadlineEndOfDay(value) { var parts = value.split("-").map(Number); return new Date(parts[0], parts[1] - 1, parts[2], 23, 59, 59, 0).toISOString(); }
  ...
  function render() {
    ...
    $("ami-title").textContent = hierarquia && hierarquia.nome || julgamento.config_nome || julgamento.hierarquizacao_codigo;
    $("ami-subtitle").textContent = (julgamento.config_codigo || julgamento.hierarquizacao_codigo) + " · criado em " + new Date(julgamento.criadoEm).toLocaleDateString("pt-BR");
    ...
    $("ami-config-details").innerHTML = "<div><span>Prazo</span><strong>" + deadlineLabel(julgamento.valido_ate) + "</strong></div><div><span>Especialistas</span><strong>" + julgamento.convites.length + "</strong></div><div><span>Respostas</span><strong>" + respostas.length + "</strong></div>";
    ...
    window.renderTabelaColaborativa("ami-workspace-database", julgamentos, { selectedId: id, linksFor: function (j) { return { workspace: appUrl("restrict", "/analise-multicriterio/julgamentos/" + encodeURIComponent(j.id) + "/"), public: appUrl("public", "/analise-multicriterio/" + encodeURIComponent(j.token) + "/") }; } });
  }
  function load() { return Promise.all([api(appPrefix + "/" + "api/ahp/comparacao-colaborativa/ambientes/" + id), api(appPrefix + "/" + "api/ahp/comparacao-colaborativa/ambientes/" + id + "/respostas"), api(appPrefix + "/" + "api/ahp/comparacao-colaborativa/configuracoes"), api(appPrefix + "/" + "api/ahp/comparacao-colaborativa/ambientes")]).then(function (v) { julgamento = v[0]; respostas = v[1]; hierarquia = v[2].find(function (h) { return h.id === (julgamento.config_id || julgamento.hierarquizacao_id); }); julgamentos = v[3]; render(); }); }
