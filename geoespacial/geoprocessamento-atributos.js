...
(() => {
  ...
  async function save(){const d=current;d.busy=true;controls();try{
    ...
    let id=d.id;
    if(file){const result=await request('/api/geoespacial/bancada-arquivos/salvar',{arquivo:file.arquivo,revisao:file.revisao,geojson,nome:file.nome+' — edição'});window.gpArquivos.adicionar(result);id=result.id;}
