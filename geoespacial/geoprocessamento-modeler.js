(function () {
  ...
  function open(type, existing=null, seed=null, draft=null){
    ...
    if(existing?.diagrama?.nos?.length){ editor.nodes=structuredClone(existing.diagrama.nos||[]);editor.nodes.forEach(node=>{if(["algorithm","function"].includes(node.kind))node.label=displayName(node.kind,node.ref,node.label)}); editor.edges=structuredClone(existing.diagrama.conexoes||[]); }
  ...
  function processFeedback(editor,action){
    if(window.gpFeedback){const proc=window.gpFeedback.processo(editor.title);return {
      step:(message,status)=>proc.passo(message,status),
      complete:message=>proc.concluir({message}),fail:message=>proc.concluir({type:"error",message})};}
  ...
  function notify(message){if(window.gpFeedback){window.gpFeedback.info(message,"Bancada de geoprocessamento");return;}const status=$("#gp-save-state");status.textContent=message;clearTimeout(notify.timer);notify.timer=setTimeout(()=>status.textContent="Ambiente local",4500);}
  async function close(id){const editor=editors.get(id);if(editor?.dirty&&!(window.gpFeedback?await window.gpFeedback.confirmar({message:"Fechar sem salvar as alterações?",danger:true,confirmLabel:"Fechar sem salvar"}):confirm("Fechar sem salvar as alterações?")))return;editors.delete(id);$(`[data-document-tab="${id}"]`)?.remove();$(`[data-model-document="${id}"]`)?.remove();activate("mapa");}
