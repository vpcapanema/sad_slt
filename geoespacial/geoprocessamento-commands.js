(function () {
  ...
  function notify(text) { if(window.gpFeedback){window.gpFeedback.Notify.info("Bancada de geoprocessamento",text,{duration:5000});return;}const status = $("#gp-save-state"); status.textContent = text; clearTimeout(notify.timer); notify.timer = setTimeout(() => status.textContent = "Ambiente local", 3500); }
