(function(){
  ...
  function log(msg,type=""){if(window.gpFeedback&&type){const tipo=type==="ok"?"success":type==="error"?"error":"info";window.gpFeedback.Notify[tipo]("Bancada de geoprocessamento",msg,tipo==="error"?{}:{duration:5000});}const el=$("#gp-log"),time=new Date().toLocaleTimeString();el.insertAdjacentHTML("beforeend",`<div class="log-${type}">[${time}] ${escapeHtml(msg)}</div>`);el.scrollTop=el.scrollHeight}
  ...
  async function executeOp(op,form){
    ...
    payload.filtros_camadas=Object.fromEntries(Object.entries(state.layerFilters||{}).filter(([id])=>Object.values(payload).some(value=>value===id||Array.isArray(value)&&value.includes(id))));
  ...
  function showLibrary(kind,intent=null){
    ...
    if(["edit","validate","run"].includes(intent)){
      const verb={edit:"editar",validate:"validar",run:"executar"}[intent];
      $("#gp-right-title").textContent=`Selecione ${isFn?"uma função":"um fluxo"} para ${verb}`;
      $("#gp-editor-view .editor-head h2").textContent=`Selecione ${isFn?"a função":"o fluxo"} para ${verb}`;
      $$("#gp-editor-view [data-edit-definition], #gp-editor-view [data-validate-definition], #gp-editor-view [data-run-definition], #gp-editor-view [data-delete]").forEach(button=>{button.hidden=!button.hasAttribute(`data-${intent}-definition`);});
    }
    ...
    $("#gp-editor-view").onclick=async event=>{
      ...
      try{
        ...
        if(del){
          ...
          del.disabled=true;
          const item=list.find(value=>value.id===del.dataset.delete);
          const approved=window.gpFeedback
            ?await window.gpFeedback.ProcessFeedback.confirmar({title:`Excluir ${isFn?"função":"fluxo"}`,message:`Excluir a definição “${item?.nome||del.dataset.delete}”?`,warning:"Fluxos que dependem desta definição poderão deixar de executar.",danger:true,confirmLabel:"Excluir definição"})
            :confirm(`Excluir a definição “${item?.nome||del.dataset.delete}”?`);
          if(!approved){del.disabled=false;return;}
          const response=await fetch(`${API}/${endpoint}/${del.dataset.delete}`,{method:"DELETE"});if(!response.ok)throw new Error(`HTTP ${response.status}`);await refreshDefinitions();showLibrary(kind)}
      ...
      }catch(error){if(del)del.disabled=false;log(error.message,"error");$("#gp-log").classList.add("open")}
    };
  }
  ...
  async function showAttributes(layerId,offset=0){
    ...
    try{
      ...
      await window.gpCommands?.refreshLayerFilter(layerId);
  ...
  async function deleteLayerFromSystem(layerId){
    if(!layerId)return;
    const layer=state.layers.find(item=>item.id===layerId);
    if(!(window.gpFeedback?await window.gpFeedback.ProcessFeedback.confirmar({title:"Excluir camada",message:`Excluir “${layer?.nome||layerId}” definitivamente do sistema?`,danger:true,confirmLabel:"Excluir"}):window.confirm(`Excluir “${layer?.nome||layerId}” definitivamente do sistema?`)))return;
