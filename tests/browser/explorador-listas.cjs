...
assert.equal((await painel.locator('[data-pfs="progress-title"]').innerText()).trim(),titulo.trim());
assert.equal(await painel.locator('[data-pfs="task-name"]').innerText(),'Carregar camadas selecionadas');
assert.equal(await painel.locator('[data-pfs="progress-percent"]').innerText(),'0%');
assert.match(await painel.locator('[data-pfs="task-desc"]').innerText(),/rios/);
assert.match(await painel.locator('[data-pfs="task-desc"]').innerText(),/Lendo a camada/);
pendentes.get(layers[0].id)();
await p.waitForFunction(()=>document.querySelector('[data-pfs="progress-percent"]')?.textContent==='50%');
assert.doesNotMatch(await painel.locator('[data-pfs="task-desc"]').innerText(),/rios/);
assert.match(await painel.locator('[data-pfs="task-desc"]').innerText(),/lagos/);
