...
await p.locator('#ea-base-browse').click();assert.equal(await p.locator('#ea-category-select').getAttribute('aria-invalid'),'true');assert.equal(await p.locator('#slt-feedback-backdrop').count(),0);
...
assert.equal(await p.locator('#slt-feedback-backdrop').count(),0);
await p.locator('#ea-staging-confirmar').click();await p.waitForFunction(()=>!document.querySelector('#ea-run').disabled);
assert.equal(await p.locator('#slt-feedback-backdrop').count(),0,'Confirmação redundante de bases removida');
await p.locator('.slt-fb-process-panel:not([data-processando]) [data-fb-close]').click();
await p.locator('#ea-config-salvar').click();await p.locator('[data-fb-input]').fill('Configuração');await p.locator('[data-fb-confirmar]').click();await p.locator('.slt-fb-notice').filter({hasText:'Configuração salva'}).count();assert.equal(await p.locator('#slt-feedback-backdrop').count(),0);
...
assert.equal(await p.locator('#slt-feedback-backdrop').count(),0);
const proc=p.locator('.slt-fb-process-panel').filter({hasText:'Extração em andamento'});await proc.getByRole('button',{name:'Ver resultados'}).click();
await p.locator('#ea-results [data-view="attributes"]').click();await p.getByText('1–100 de 101 registros.',{exact:false}).waitFor();assert.equal(await p.locator('#slt-feedback-backdrop').count(),0);assert.equal(await p.locator('.slt-fb-process-panel').count(),0);
falharTabela=true;await p.getByRole('button',{name:'Próxima',exact:true}).click();await p.locator('#ea-results-content .slt-fb-context').waitFor();
assert.match(await p.locator('#ea-results-content .slt-fb-context').textContent(),/Tabela temporariamente/);
falharTabela=false;await p.getByRole('button',{name:'Tentar novamente'}).click();await p.getByText('101–101 de 101 registros.',{exact:false}).waitFor();
assert.deepEqual(errors,[]);console.log('PASS: página real com validação no campo, leitura contextual, confirmação apenas da execução, salvamento discreto e tabela com recuperação.');await b.close();
