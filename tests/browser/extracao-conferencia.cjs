...
(async()=>{
 ...
 const fc={type:'FeatureCollection',features:[{type:'Feature',properties:{valor:1,tipo:'Escola'},geometry:{type:'Point',coordinates:[-46,-23]}}]};
 ...
 if(path.endsWith('/arquivo-mapa'))return send({...catalog.camadas.find(c=>c.id===r.request().postDataJSON().id),geojson:fc,campos:[{nome:'valor'},{nome:'tipo'}]});
 ...
 await escolher('#ea-input-browse','entrada');
 await p.waitForFunction(()=>document.querySelectorAll('#ea-identificacao-camadas tr').length===1);
 assert.equal(await p.locator('#ea-input-preview-layers .ea-preview-layer').count(),1,'Arquivo do catálogo também aguarda confirmação');
 await p.locator('#ea-identificacao-camadas [data-id="campo"]').selectOption('valor');
 await p.locator('#ea-identificacao-camadas [data-id="categoria"]').selectOption('tipo');
 await p.locator('#ea-identificacao-confirmar').click();
 await p.waitForFunction(()=>document.querySelectorAll('#ea-input-preview-layers .ea-preview-layer').length===2);
 await p.locator('#ea-identificacao-camadas [data-id="categoria"]').selectOption('');
 await p.waitForFunction(()=>document.querySelectorAll('#ea-input-preview-layers .ea-preview-layer').length===1);
 await p.locator('#ea-input-clear').click();
 ...
 assert.equal(await p.locator('#ea-identificacao-camadas select').count(),4);
 assert.equal(await p.locator('#ea-input-preview-layers .ea-preview-layer').count(),1,'Demandas aguardam confirmação, só a base está na prévia');
 for(const select of await p.locator('#ea-identificacao-camadas [data-id="categoria"]').all())await select.selectOption('tipo');
 assert.equal(await p.locator('#ea-run-inputs tr').count(),0,'Prévia não entra na execução');
 assert(await p.locator('#ea-run-individual').isDisabled());
 assert(await p.locator('#ea-run-batch').isDisabled());
 ...
 await p.waitForFunction(()=>document.querySelectorAll('#ea-input-preview-layers .ea-preview-layer').length===3);
 ...
 assert(await p.locator('#ea-run-individual').isDisabled(),'Individual indisponível com duas demandas');
 assert(await p.locator('#ea-run-batch').isChecked());
 await p.locator('#ea-run-batch').check();
 await p.waitForFunction(()=>!document.querySelector('#ea-run').disabled);
 await p.locator('.ea-checkup').screenshot({path:'/tmp/sicard-conferencia-compacta.png'});
 const radio=await p.locator('#ea-run-batch').boundingBox();assert(radio.height<=16);
 ...
 assert(await p.locator('#ea-run-batch').isDisabled(),'Lote indisponível com uma demanda');
 assert(await p.locator('#ea-run-individual').isChecked());
 await p.locator('#ea-run-individual').check();
 ...
 assert.equal(pedidos[0].entradas[0].config.camadas['um.gpkg::1'].categoria_demanda,'tipo');
 assert.match(await p.locator('#ea-run-inputs').textContent(),/tipo/);
 assert.deepEqual(Object.keys(pedidos[0].entradas[0].config.camadas),['um.gpkg::1']);
 await p.setViewportSize({width:390,height:844});
 const compact=await p.locator('.ea-checkup').evaluate(n=>({width:n.clientWidth,scroll:n.scrollWidth}));assert(compact.scroll<=compact.width+1,'Conferência cabe na tela estreita');
