...
const p=await b.newPage({viewport:{width:1440,height:1000}});const errors=[],requests=[];let saved,polls=0,municipalCatalog=0,municipalPayload;
...
await p.route('**/api/**',async r=>{
 ...
 if(path.endsWith('/municipal/ambiental/export')){
  municipalPayload=r.request().postDataJSON();catalog.camadas.push({id:'municipal',nome:'Municipal gerada',origem:'importadas'});
  return r.fulfill({contentType:'application/zip',headers:{'X-Camada-Id':'municipal','X-Camada-Arquivo':'teste/municipal.fgb'},body:'fixture-download'});
 }
...
await p.locator('#ea-staging-list').getByText('Municipal gerada',{exact:true}).waitFor();
