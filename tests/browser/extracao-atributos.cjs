...
const p=await b.newPage({viewport:{width:1440,height:1000}});const errors=[],requests=[];let saved,polls=0;
...
const catalog={categorias:[{id:'ambiental',nome:'Ambiental'}],camadas:['entrada','base','adicional'].map(id=>({id,nome:id,origem:'importadas'}))};
...
assert.equal(errors.length,0,JSON.stringify(errors));console.log('PASS: seleção banco, exclusões, mapa real, entradas adicionais, salvar após confirmar, payload, tabela paginada, abas, download e invalidação.');await b.close();
