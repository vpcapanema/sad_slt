...
await p.goto(`${process.env.TEST_BASE_URL||'http://127.0.0.1:8083'}/restrict/geoespacial/extracao-atributos/`,{waitUntil:'domcontentloaded'});
...
fail=true;await p.locator('#ea-base-list-confirmar').click();await p.locator('#pfsConfirmOk').click();await p.locator('#pfsErrorBox.pfs-active').waitFor();assert.equal(await p.locator('[data-pfs="error-title"]').innerText(),'A lista não foi enviada à prévia');assert(!await p.locator('#ea-input-preview').isVisible());
await p.evaluate(()=>StatusFeedback.fechar());
fail=false;await p.locator('#ea-base-list-confirmar').click();await p.locator('#pfsConfirmOk').click();await p.locator('#ea-input-preview').waitFor();assert.equal(await p.locator('#ea-input-preview-layers .ea-preview-layer').count(),2);
...
await p.locator('#ea-staging-confirmar').click();await p.locator('#pfsConfirmOk').click();await p.waitForFunction(()=>document.querySelector('iframe').contentWindow.gpApp.state.layers.length===2);
...
await p.locator('#ea-base-list-editar').click();await card.getByRole('checkbox',{name:'Selecionar para excluir: a',exact:true}).check();await p.locator('#ea-base-list-excluir').click();assert.equal(await rows.count(),1);assert.equal(await p.locator('#ea-input-preview-layers .ea-preview-layer').count(),0);
...
await p.locator('#ea-base-list-confirmar').click();await p.locator('#pfsConfirmOk').click();await p.locator('#pfsProgressClose').click();await p.locator('#ea-base-list-cancelar').click();await p.waitForTimeout(1100);
assert(!await card.isVisible());assert.equal(await p.locator('#ea-input-preview-layers .ea-preview-layer').count(),0);await carregar();
