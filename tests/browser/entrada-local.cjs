...
await page.addLocatorHandler(page.locator('.notification-toast.info').first(),async()=>{await page.evaluate(()=>Notify.clearAll());});await page.addLocatorHandler(page.locator('#pfsStatusOverlay.pfs-active').filter({hasText:'Bancada de geoprocessamento'}),async()=>{await page.evaluate(()=>StatusFeedback.fechar());});
...
await page.locator('#pfsStatusOverlay .pfs-box.pfs-active .pfs-footer .pfs-btn').click();
...
await page.locator('#pfsStatusOverlay .pfs-box.pfs-active .pfs-footer .pfs-btn').click();fail=false;
...
await page.locator('#ea-staging-confirmar').click();await page.locator('#pfsConfirmOverlay').getByRole('button',{name:'Confirmar bases',exact:true}).click();await page.locator('#pfsStatusOverlay .pfs-box.pfs-active').getByRole('button',{name:'OK',exact:true}).click();
assert.equal(await page.locator('#ea-staging-confirmar').isEnabled(),false);
await page.selectOption('#ea-operation','estatisticas');
await page.locator('#ea-config-salvar').click();await page.locator('#pfsConfirmInput').fill('Config local');await page.locator('#pfsConfirmOk').click();await page.waitForTimeout(150);
assert.equal(saved[0].entradas.length,0);assert.equal(JSON.stringify(saved).includes('conteudo_base64'),false);
await page.locator('#ea-run').click();await page.locator('#pfsConfirmOverlay').getByRole('button',{name:'Executar extração',exact:true}).click();await page.locator('#pfsStatusOverlay .pfs-box.pfs-active').getByRole('button',{name:'OK',exact:true}).click();
...
await page.waitForFunction(()=>document.querySelector('#ea-input-select').value==='local:2');await page.locator('#pfsPartialBox.pfs-active .pfs-footer .pfs-btn').click();
...
await base2();
await page.locator('#ea-staging-confirmar').click();await page.locator('#pfsConfirmOk').click();await page.locator('#pfsStatusOverlay .pfs-box.pfs-active').getByRole('button',{name:'OK',exact:true}).click();
await page.locator('#ea-run').click();await page.locator('#pfsConfirmOk').click();await page.locator('#pfsStatusOverlay .pfs-box.pfs-active').getByRole('button',{name:'OK',exact:true}).click();
