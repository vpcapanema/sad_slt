...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(await territorial.locator('[data-kpi]').allTextContents(),['32','11','16','2']);
    await page.evaluate(()=>window.originalPanel=document.querySelector('#ea-panel-territorial'));
    await territorial.locator('[data-filter=entrada]').selectOption('Ferrovias');await readyFor('#ea-panel-territorial');
    assert.equal(await territorial.locator('[data-ranking="risco"] tbody tr').count(),2);
    await territorial.locator('[data-ranking="restricao"] tbody button').first().click();
    assert.match(await territorial.locator('[data-role="selection-title"]').textContent(),/D-0.*Ferrovias/);
    assert.match(await territorial.locator('[data-role="occurrences"]').textContent(),/12 m²/);
    assert.match(await territorial.locator('[data-role="selection-attributes"]').textContent(),/Área protegida/);
    await territorial.locator('[data-filter=categoria]').selectOption('risco');
    assert.match(await territorial.locator('[data-role="selection-attributes"]').textContent(),/Inundação/);
    await territorial.locator('[data-filter=atributo]').selectOption('nome');
    assert.match(await territorial.locator('[data-role="occurrences"]').textContent(),/Inundação/);
    await territorial.getByRole('button',{name:'Limpar seleção',exact:true}).click();await readyFor('#ea-panel-territorial');
    await territorial.getByRole('button',{name:'Próxima página',exact:true}).click();await readyFor('#ea-panel-territorial');
    assert.equal(await territorial.locator('[data-ranking="risco"] tbody tr').count(),7);
