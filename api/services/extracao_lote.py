...
def entradas_do_lote(entradas):
    ...
    for entrada in entradas:
        ...
        for parte in partes:
            ...
            categoria = config.get('categoria_pontos')
            if categoria and categoria not in dados.columns:
                raise ValueError(f'{nome}: campo de categoria dos pontos inexistente.')
...
def executar(entradas, categorias, operacao, progress, finalidades=None):
    ...
    for i, entry in enumerate(entradas):
        ...
        if modo == 'enriquecimento':
            ...
            if not recorte:
                raise ValueError(f"{entry['nome']}: selecione a base de recorte do Identity na seção 1.3.")
        ...
        for n, frame in res['camadas'].items():
            ...
            schemas.update(dicionario=[d for d in campos if d['camada']==remap[n]],
                entrada_id=entry['chave'], entrada_nome=entry['nome'], campo_categoria_pontos=config.get('categoria_pontos'))
