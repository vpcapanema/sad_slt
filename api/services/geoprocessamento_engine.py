...
class GeoprocessamentoEngine:
    ...
    async def rasterize(self, p: dict[str, Any]) -> dict[str, Any]:
        ...
        values = gdf[field] if field else np.ones(len(gdf)); data = rasterize(zip(gdf.geometry, values), out_shape=(height, width), transform=transform, fill=cast(Any, float(p.get("valor_preenchimento", 0))), all_touched=bool(p.get("processar_todas_celulas_tocadas", False)), dtype="float32")
        if data is None:
    ...
    async def distance(self, p: dict[str, Any]) -> dict[str, Any]:
        ...
        base = await self.rasterize(q); resolution = float(q["resolucao_raster"]); distances = distance_transform_edt(self._raster(base["raster_id"]) == 0, return_distances=True, return_indices=False)
    ...
    async def density(self, p: dict[str, Any]) -> dict[str, Any]:
        ...
        model = KernelDensity(kernel=cast(Any, kernel), bandwidth=float(p.get("largura_kernel", 1000))).fit(samples)
    ...
    async def sample(self, p: dict[str, Any]) -> dict[str, Any]:
        ...
        for geom in points.geometry:
            row,col=rowcol(profile["transform"],geom.centroid.x,geom.centroid.y); out.append(float(data[row,col]) if 0<=row<data.shape[0] and 0<=col<data.shape[1] else None)
        return {"valores":out}
