...
Cada geração recebe uma pasta exclusiva em `data/geoespacial/uploads/datastorage/vetor/municipios_sp_<id_curto>_<nome>/`, contendo a camada, o dicionário e os metadados. A escolha deste destino atende à integração com o acervo solicitada para este gerador. O banco registra a camada importada, categoria, execução, caminho, componentes e hashes; não recebe cópia das feições. A extração lê esta base municipal diretamente do arquivo, inclusive após reiniciar a aplicação.
